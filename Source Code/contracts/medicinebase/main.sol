...
contract main is UserRole, ManufacturerRole, DistributorRole {
  ...
  mapping (string => VMedicine) vmedicines;
  ...
  function makeMedicine(string memory _batchno, string memory _medicineName,string memory _dosage, address  _originManufacturerID,address _ownerID, string memory _FactoryName, string memory _mfgdate,string memory _expdate,State) public
...
//     address userID; // Metamask-Ethereum address of the Patient
  ...
  function packMedicine(string memory _batchno, string memory _medicineName,string memory _dosage, address  _originManufacturerID,address _ownerID, string memory _FactoryName, string memory _mfgdate,string memory _expdate,State,address _distributorID) public
  onlyDistributor 
  // Call modifier to check if batchno has passed previous supply chain stage
  //Made(_batchno)
  // Call modifier to verify caller of this function
  {
     VMedicine memory packMedicine = VMedicine({
      batchno:_batchno,  
      medicineName:_medicineName,
      dosage:_dosage,
      originManufacturerID:_originManufacturerID,// Metamask-Ethereum address of the Manufacturer
      ...
      mfgdate:_mfgdate,//mfgdate
      expdate:_expdate,//expdate
      medicineState: State.Packed,
      distributorID: _distributorID
      });
    // Update the appropriate fields
    vmedicines[_batchno] = packMedicine;
    vmedicines[_batchno].medicineState = State.Packed;
  ...
  onlyUser
    ...
    {
    vmedicines[_batchno].medicineState = State.Sold;  
...
function fetchMedicineBufferTwo(string memory _batchno) public view returns 
  ...
  // Assign values to the 7 parameters
  return (
    vmedicines[_batchno].batchno,
    vmedicines[_batchno].medicineName,
    //vmedicines[_batchno].ownerID,
    //vmedicines[_batchno].originManufacturerID,
    vmedicines[_batchno].dosage,
    vmedicines[_batchno].FactoryName,
    vmedicines[_batchno].mfgdate,
    vmedicines[_batchno].expdate,
    //vmedicines[_batchno].medicineState,
    vmedicines[_batchno].distributorID
    );
}
function fetchMedicineBufferThree(string memory _batchno) public view returns 
  (
    string memory batchno,  //the primary key batchno 
    string memory  medicineName, //Medicine name
    address ownerID,  // Metamask-Ethereum address of the current owner as the medicine moves through 3 stages
